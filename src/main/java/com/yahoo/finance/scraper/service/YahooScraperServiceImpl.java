...
public class YahooScraperServiceImpl implements YahooScraperService {
    ...
    public List<Ticker> fetchData(List<String> tickerSymbols){
        ExecutorService executorService = Executors.newFixedThreadPool(THREAD_POOL_SIZE);

        List<Callable<Ticker>> tasks = new ArrayList<>();
        for (String symbol : tickerSymbols) {
            tasks.add(() -> scrapeTickerData(symbol));
        }

        try {
            List<Future<Ticker>> futures = executorService.invokeAll(tasks);
            List<Ticker> tickers = new ArrayList<>();

            for (Future<Ticker> future : futures) {
                try {
